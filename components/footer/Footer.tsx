  ...
  return (
    <footer className="relative w-full py-16 bg-gradient-to-b from-black to-[#050510] overflow-hidden">
      ...
      >
            ...
            <div className="flex gap-3 pt-2">
              {[
                ...
                <motion.a
                  ...
                  aria-label={t.footer.social.ariaLabel[social.icon as keyof typeof t.footer.social.ariaLabel]}
                  initial={{ opacity: 0, scale: 0.8 }}
                  whileInView={{ opacity: 1, scale: 1 }}
                  transition={{ delay: i * 0.1 }}
                  whileHover={{ scale: 1.2, y: -4 }}
                  whileTap={{ scale: 0.9 }}
                  className="w-10 h-10 rounded-xl bg-gradient-to-br from-white/10 to-white/5 border flex items-center justify-center transition-all duration-300 cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/60 focus-visible:ring-offset-2 focus-visible:ring-offset-black"
                ...
                >
                  {social.icon === "github" && (
                    <svg className="w-5 h-5 text-gray-400 hover:text-cyan-300 transition-colors" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  ...
                  {social.icon === "email" && (
                    <svg className="w-5 h-5 text-gray-400 hover:text-cyan-300 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        ...
        {/* SEO keywords: natural line + expandable tags (all in DOM for crawlers, UI stays clean) */}
        <motion.section
          ...
          className="mb-10"
          aria-label="Related keywords for search"
        >
          <p
            className="text-xs text-gray-500/90 text-center max-w-2xl mx-auto leading-relaxed mb-4"
            style={{ color: `rgba(${colors.accent}, 0.35)` }}
          >
            {t.footer.seo.summaryLine}
          </p>
          <details className="group">
            <summary className="list-none cursor-pointer text-center">
              <span
                className="inline-flex items-center gap-1.5 text-xs font-medium transition-colors hover:opacity-80"
                style={{ color: `rgba(${colors.accent}, 0.5)` }}
              >
                <span className="group-open:rotate-180 transition-transform inline-block">
                  ▼
                </span>
                {t.footer.seo.expandLabel}
              </span>
            </summary>
            <div className="mt-4 flex flex-wrap justify-center gap-2">
              {SEO_KEYWORDS.map((keyword, i) => (
                <span
                  key={`${keyword}-${i}`}
                  className="px-2.5 py-1 text-[11px] rounded-md border opacity-80"
                  style={{
                    borderColor: `rgba(${colors.accent}, 0.15)`,
