...
function Hero() {
  ...
  return (
    <section className="relative w-full h-screen bg-black overflow-hidden hero-cursor-area">
      ...
      <button
        onClick={switchLang}
        className="fixed top-6 right-6 z-[10001] flex items-center gap-2 px-4 py-2 text-xs font-semibold rounded-full backdrop-blur-md bg-white/10 border border-white/20 text-cyan-300 hover:bg-cyan-400/20 transition cursor-pointer"
      >
        <Languages size={14} />
        {locale === "vi" ? "EN" : "VI"}
      </button>
